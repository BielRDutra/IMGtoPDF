...
class ImagewToPDFConverter:
    ...
    def initialize_ui(self):
        ...
        select_images_button = tk.Button(self.root, text="Selecionar imagens", command=self.select_images)
        select_images_button(pady=(0, 10))
        ...
        convert_button = tk.Button(self.root, text="Converter para PDF", command=self.convert_images_to_pdf)
        convert_button.pack(pady=(20, 40))

    def select_images(self):
        self.image_paths = filedialog.askopenfilenames(title="Selecionar Imagens", filetypes=[("Image files", "*.png;*.jpg;*.jpeg")])
        self.update_selected_images_listbox()

    def update_selected_images_listbox(self):
        self.selected_images_listbox.delete(0, tk.END)

        for images_path in self.image_paths:
            _, image_path = os.path.split(image_path)
            self.selected_images_listbox.insert(tk.END, image_path)

    def coinvert_images_to_pdf(self):
        ...
        for image_path in self.image_paths:
            ...
            y_centered = (791 - new_height) / 2

            pdf.setFillColorRGB(255, 255, 255)
            pdf.rect(0, 0, 612, 791, fill=True)
...
def main():
    ...
    converter = ImagewToPDFConverter(root)
